...
class QueueViewModel(private val lobbyService: LobbyService): ViewModel() {
    ...
    private suspend fun fetchLobbyInfo(lobbyID: ID, userToken: String): LobbyInformation {
        return coroutineScope {
            ...
            return@coroutineScope result.await() ?: throw IllegalArgumentException("LobbyInfo is null")
        }
    }

    fun waitForFullLobby(lobbyID: ID, userToken: String, onContinuation: ()-> Unit){
